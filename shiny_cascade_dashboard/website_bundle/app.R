# Modular entry: propagation graph in R/propagation_graph.R
# Original (deployed): app.R — unchanged
# Run: shiny::runApp("app_modular.R")
...
propagation_graph_script <- file.path(getwd(), "R", "propagation_graph.R")
if (!file.exists(propagation_graph_script)) {
  stop("Missing R/propagation_graph.R — run this app from shiny_cascade_dashboard/.")
}
source(propagation_graph_script, local = FALSE)
...
edge_data <- load_propagation_edge_data(
  cascade_edges_csv = cascade_edges_csv,
  data_model_path = data_model_path,
  read_csv = read_csv_safe
)

build_graph_for_tweet <- function(tweet_id) {
  build_propagation_graph_for_tweet(edge_data, tweet_id)
}
